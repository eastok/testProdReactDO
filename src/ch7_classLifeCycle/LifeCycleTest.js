...
class LifeCycleTest extends Component {
  ...
  // props.color 가 들어 있다.
  constructor(props) {
    // 클래스가 혼자 동작 못함, 반드시 누군가 부모를 상속받아야 함.
    // 부모를 초기화 해주기
    super(props);
    console.log("1번 constructor 호출");
  }
  ...
  // 4번째 호출 : 마운트가 완료됐어요
  componentDidMount() {
    console.log("4번째 호출 : componentDidMount 완료됐어요.");
  }

  // 업데이트 변경을 할까요?
  shouldComponentUpdate(nextProps, nextState) {
    console.log("2번째 호출 : shoulComponentUpdate", nextProps, nextState);
    // 숫자가 일의 자리가 4이면 리렌더링 안함.
  ...
  getSnapshotBeforeUpdate(prevProps, prevState) {
    console.log("getSnapshotBeforeUpdate 호출.", prevProps.color, this.color);
  ...
  render() {
    console.log("순서 3번째 render 호출 : 마운트 또는 업데이트시 호출");
