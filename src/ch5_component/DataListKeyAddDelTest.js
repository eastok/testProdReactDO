...
const DataListKeyAddDelTest = () => {
  ...
  // 설정 후 붙이기 작업
  const onClickText = () => {
    const nextTestArray = testArr.concat({
      id: nextId,
      // inputText, 입력된 내용이 계속 변경을 감지하면서, 최종 단어가 엄데이트가 됨.
      text: inputText,
    });
    // id가 추가 되어서 1씩 증가하는 로직
    setNextId(nextId + 1);
    // 배열에 새로운 요소가 추가된 배열을 업데이트
    setTestArr(nextTestArray);
    // 입력 완료 했으니, 입력창 비우기.
    setInputText("");
  };
  ...
  const onRemoveText = (id) => {
    // !== 틀리면 참, 같으면 거짓
    // == 같으면 참, 틀리면 거짓
    // filter(콜백함수(조건)), filter 조건이 참이 되는 요소만 뽑아서, 새로운 배열을 만든다.
    // 예) 원본 id : 1~4, 삭제할 아이디가 : 3번
    // 1~4 검사
    // 1 !== 3 true -> 1 필터됨, 결과 배열 [1]
    // 2 !== 3 true -> 2 필터됨, 결과 배열 [1, 2]
    // 3 !== 3 false -> 3 필터 안 됨, 결과 배열 [1, 2]
